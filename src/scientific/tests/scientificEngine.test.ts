...
export function runAllScientificTests(): { results: TestResult[]; summary: { total: number; passed: number; failed: number } } {
  ...
  {
    const config = createSeriesConfiguration('GLOSS', 2, 1, ruleSet, { justification: 'Renforcement du plan de mesure' });
    const raw: GlossRawData = {
      series: [
    ...
    record(14, 'Brillance 2 × 1 -> ADAPTED_JUSTIFIED', 'Protocole Brillance', passed, 'ADAPTED_JUSTIFIED (6 mesures)', `${res.computed.protocolStatus} (${res.computed.totalReadings} mesures)`);
  }
